            ...
            create() {
              ...
              const timer = setInterval(() => {
                ...
                for (let i = 0; i < Math.floor(QUANTITY / 3); i++) {
                  // 1階層目のスタイル生成
                  const keyframe = 'moving-' + (Math.floor(Math.random() * 8) + 1); // 0の生成を防ぐ
                  ...
                  const rotateKeyframe = Math.floor(Math.random() * 1) ? 'rotateY' : 'rotate360';
...
$(window).on('load', function () {
  ...
  setTimeout(EachTextAnimeControl, 1500);/* アニメーション用の関数を呼ぶ*/
